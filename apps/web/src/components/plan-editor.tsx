...
// hook simples para carregar uma imagem em HTMLImageElement
function useHtmlImage(url: string | null) {
  const [img, setImg] = useState<HTMLImageElement | null>(null);
  useEffect(() => {
    if (!url) {
...
export function PlanEditor({
  ...
  // ---- fundo: upload de imagem (PNG/JPG) ----
  const bgInputRef = useRef<HTMLInputElement>(null);
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="rounded-md border border-border px-3 py-1.5 text-xs text-muted hover:text-paper"
          >
            {state.background ? "Trocar planta" : "Carregar planta (imagem)"}
          </button>
          <button
            onClick={() => {
              setMode(mode === "calibrate" ? "select" : "calibrate");
