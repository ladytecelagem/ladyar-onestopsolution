...
import {
  saveCanvasState,
  runCanvasAnalysis,
} from "@/app/(app)/projects/editor-actions";
import {
  type CanvasState,
  type CanvasElement,
  EMPTY_CANVAS,
...
export function PlanEditor({
  ...
  // Resultado do botão "Calcular" — RT60, score, custo. Não persiste no client;
  // ao reabrir o projeto, o painel de análise do /projects/:id mostra o salvo.
  ...
  // undo/redo: histórico em ref (não dispara render por si). forceHist atualiza botões.
  const histRef = useRef<{ past: CanvasState[]; future: CanvasState[] }>({
    past: [],
    future: [],
  });
  const [, forceHist] = useState(0);
  function commitState(
    updater: CanvasState | ((s: CanvasState) => CanvasState)
  ) {
    setState((prev) => {
      const next =
        typeof updater === "function"
          ? (updater as (s: CanvasState) => CanvasState)(prev)
          : updater;
  ...
  const snap = (v: number) => (snapOn ? Math.round(v / GRID) * GRID : v);
  ...
  function onDrop(e: React.DragEvent) {
    ...
    commitState((s) => ({ ...s, elements: [...s.elements, el] }));
    setSelectedId(el.id);
  }
  ...
  function duplicateSelected() {
    ...
    const src = state.elements.find((e) => e.id === selectedId);
    if (!src) return;
    ...
    commitState((s) => ({ ...s, elements: [...s.elements, ne] }));
    setSelectedId(ne.id);
  }

  const calculate = useCallback(async () => {
    setCalculating(true);
    // salva o canvas atual primeiro pra garantir que o cálculo bate com o que está na tela
    await saveCanvasState(projectId, state);
    const res = await runCanvasAnalysis(projectId, state);
    setCalculating(false);
    if (res.error) {
      alert(res.error);
      return;
    }
    if (res.result) setAnalysis(res.result);
    router.refresh();
  ...
  // teclado: Delete remove, Ctrl/Cmd+D duplica, Ctrl/Cmd+Z undo, Ctrl+Shift+Z / Ctrl+Y redo
  useEffect(() => {
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="rounded-md border border-border px-2 py-1.5 text-xs text-muted hover:text-paper disabled:opacity-30"
              aria-label="Desfazer"
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              onClick={calculate}
              disabled={calculating || saving || state.elements.length === 0}
              className="rounded-lg border border-gold px-4 py-1.5 text-xs font-medium text-gold hover:bg-gold hover:text-ink disabled:opacity-40"
            >
              {calculating ? "Calculando…" : "Calcular RT60"}
            </button>
            <button
              onClick={save}
              disabled={saving}
        ...
        >
          ...
          >
            <Layer ref={layerRef}>
              ...
              {/* grade de encaixe */}
              {snapOn &&
                Array.from({ length: Math.floor(STAGE_W / GRID) + 1 }).map(
                  (_, i) => (
                    <Line
                      key={"gv" + i}
                      points={[i * GRID, 0, i * GRID, STAGE_H]}
                      stroke="rgba(111,181,138,0.15)"
                      strokeWidth={1}
                      listening={false}
                    />
                  )
              ...
              {snapOn &&
                Array.from({ length: Math.floor(STAGE_H / GRID) + 1 }).map(
                  (_, i) => (
                    <Line
                      key={"gh" + i}
                      points={[0, i * GRID, STAGE_W, i * GRID]}
                      stroke="rgba(111,181,138,0.15)"
                      strokeWidth={1}
                      listening={false}
                    />
        ...
        {analysis && (
          <div className="mt-4 rounded-lg border border-gold/40 bg-surface p-4">
            <div className="mb-2 flex items-baseline justify-between">
              <h4 className="text-xs uppercase tracking-wide text-gold">
                Resultado do cálculo
              </h4>
              {!analysis.used_briefing && (
                <span className="text-[10px] text-muted">
                  sem briefing — estimativas padrão
                </span>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
              <Metric label="RT60 atual" value={analysis.rt60_before.toFixed(2) + " s"} />
              <Metric
                label="RT60 com tratamento"
                value={analysis.rt60_after.toFixed(2) + " s"}
                accent
              />
              <Metric label="Alvo" value={analysis.rt60_target.toFixed(2) + " s"} />
              <Metric label="Score" value={analysis.score_before + " → " + analysis.score_after} />
              <Metric label="Volume" value={analysis.volume_m3.toFixed(0) + " m³"} />
              <Metric
                label="Área tratada"
                value={analysis.treated_area_m2.toFixed(1) + " m²"}
              />
              <Metric label="Itens" value={String(analysis.items_count)} />
              <Metric
                label="Orçamento"
                value={"R$ " + analysis.total_cost.toLocaleString("pt-BR")}
                accent
              />
            </div>
          </div>
...
function Metric({
  ...
  return (
    <div>
      ...
      <p className={accent ? "text-sm font-medium text-gold" : "text-sm text-paper"}>
        {value}
      </p>
    </div>
  );
