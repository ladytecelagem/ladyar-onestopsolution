...
// ---- versão baseada em elementos posicionados no canvas (Fase C do editor) ----
// Em vez de estimar quanto material seria necessário, calcula o RT60 após
// considerar EXATAMENTE os produtos que o arquiteto colocou na planta.

export type CanvasElementMin = {
  w_m: number;
  h_m: number;
  nrc: number | null;
};

export function analyzeRoomWithCanvas(
  floorAreaM2: number,
  roomType: RoomType,
  briefing: Briefing | null,
  canvasElements: CanvasElementMin[]
): AcousticResult {
  const b = briefing ?? null;
  const usedBriefing = !!b;
  ...
  // absorção EXTRA real dos produtos posicionados
  let treatedAreaReal = 0;
  let extraAbsorption = 0;
  for (const el of canvasElements) {
    const area = (el.w_m ?? 0) * (el.h_m ?? 0);
    const nrc = el.nrc ?? 0;
    treatedAreaReal += area;
    // delta: NRC do produto menos absorção da superfície "nua" que ele cobre
    extraAbsorption += area * Math.max(0, nrc - BARE_ALPHA_DEFAULT);
  }

  const absorptionAfter = absorptionBefore + extraAbsorption;
  const rt60After =
    absorptionAfter > 0 ? (0.161 * volume) / absorptionAfter : rt60Before;
  ...
  return {
    ...
    score_before: scoreFromRt60(rt60Before, target),
    score_after: scoreFromRt60(rt60After, target),
    treated_area_m2: round(treatedAreaReal),
    used_briefing: usedBriefing,
  };
}
